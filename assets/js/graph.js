...
// Render only the organization node initially
var orgNode = graphData.find(d => d.type === 'organization');
var nodesData = [orgNode];
var linksData = [];
...
// Import the zoom functionality from zoom.js
zoomInAndOut(svg, orgNode, function() {
    // Load the function nodes after the zoom in and out action
    var functionNodes = graphData.filter(d => d.type === 'function');
    nodesData = nodesData.concat(functionNodes);
    linksData = linksData.concat(functionNodes.map(functionNode => ({source: orgNode.id, target: functionNode.id})));
